...
import { getOrders, getInvoice, getShops, regenerateInvoice, addInvoicePayment } from '../../api'
import { PageLayout, PageHeader, Button, SearchBar, DataTable, Dialog as Modal } from '../../components/DesignSystem'
...
function InvoiceView({ order, invoice, storeMap, onRegenerate, regenerating, onAddPayment, settling }) {
  ...
  return (
    <div>
      ...
      {/* Order Info */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 bg-gray-50 rounded-lg mb-4 text-sm">
        <div>
...
function PaymentForm({ invoiceId, remaining, onAddPayment, settling }) {
  ...
  const handleSubmit = () => {
    ...
    if (parsed > remaining + 0.005) {
      alert(`Payment amount cannot exceed the remaining balance ($${Number(remaining).toFixed(2)}).`)
      return
    }
